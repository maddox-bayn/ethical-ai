...
integrating and use os AI in my task: i have used AI as an assistance not a tool for code generation, and now instead of "Give me the code and explain it " i switch to "Here is my attempt. Where is my reasosning flawed", instead of "AI write for me" to AI sharpens me", in cases when i hit a waal or can't think through, i research concept to get full fundamental understanding or for conceptual understanding  theni can try and try again if failed to generate the code.
# Do you ask AI for solutions before trying yourself?
when i am giving a task to perform instead of "AI generate the code for me", firstly i get a step by step breakdown (algorithms) of the task, i get full understanding of concept how i work it's edge cases or trade-offs that i may mis then after implementation in code that it works then i give the AI for more better approach, time complexity, space complexity, and it efficiency in real world scale.
...
when practice meet opportunity there will be success, when i used AI rightly i find it hard to fail my exams but when used wrongly to learn surely  there is failure ahead, so success or failure in an exam or interview depend on the level of how right or wrong AI is used during preparation
...
Using This approach was not easy but it made me understood the complex task, breaking it down into small task and then solve, create psuedocode for your task and i try codin it it appear hard i try and try again untill i have done all edge cases, debug and check for any trade offs or better approach.
